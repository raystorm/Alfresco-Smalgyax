/**
 * Upload Smalgyax Document GET method
 */
function main()
{
   var destination = page.url.args.destination
   ...
   for (var i = 0, sort, label; i < sortables.size(); i++)
   {
      ...
      if (label == null)
      {
         ...
         if (label != null) { label = msg.get(label); }
      }
   ...
   model.searchSort = (page.url.args["s"] != null)  ? 
                                page.url.args["s"] : (sortFields.length !== 0 ? sortFields[0].type : "");
