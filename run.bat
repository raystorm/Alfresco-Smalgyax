...
:build_share
    ...
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f smalgyax-share
	call %MVN_EXEC% clean package -pl smalgyax-share,smalgyax-share-docker
EXIT /B 0
:build_acs
    docker-compose -f "%COMPOSE_FILE_PATH%" kill smalgyax-acs
    docker-compose -f "%COMPOSE_FILE_PATH%" rm -f smalgyax-acs
	call %MVN_EXEC% clean package -pl smalgyax-integration-tests,smalgyax-platform,smalgyax-platform-docker
EXIT /B 0
:tail
...
EXIT /B 0
:prepare-test
    call %MVN_EXEC% verify -DskipTests=true -pl smalgyax-platform,smalgyax-integration-tests,smalgyax-platform-docker
EXIT /B 0
:test
    call %MVN_EXEC% verify -pl smalgyax-platform,smalgyax-integration-tests
EXIT /B 0
:purge
